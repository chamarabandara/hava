...
(function (window, $) {
        ...
        self.assignEvents = function () {
            if (grid.config.jqueryUIDraggable && !grid.config.enablePinning) {
                ...
                grid.$topPanel.on('$destroy', function () {
                    ...
                    if (grid.$topPanel != null) grid.$topPanel.off('mousedown');
    ...
    var ngFooter = function ($scope, grid) {
        //add dynamic function by chamara
        var filterFunc = $scope.pagingOptions.filterFunction + "()";
       
        $scope.updateGrid = function () {
          //  filterFunc.apply($scope.pagingOptions);

            $scope.$eval(filterFunc);
        };
        ...
        $scope.paginationSummary = function () {
            return 'Showing ' + Math.min(((($scope.pagingOptions.pageNumber - 1) * $scope.pagingOptions.pageSize) + 1), $scope.pagingOptions.totalRows) + '-' + Math.min($scope.pagingOptions.pageNumber * $scope.pagingOptions.pageSize, $scope.pagingOptions.totalRows) + ' of ' + $scope.pagingOptions.totalRows + ' total.';
        };

        $scope.pageRange = function (DisplayCount) {
            var lastPage = Math.ceil($scope.pagingOptions.totalRows / $scope.pagingOptions.pageSize);
            var maxPage = Math.min(($scope.pagingOptions.pageNumber + (DisplayCount / 2)), lastPage);
            var minPage = Math.max(1, (maxPage - DisplayCount));
            maxPage = Math.min(Math.max((minPage + DisplayCount), 1), Math.max(lastPage, 1));
            var input = [];
            for (var i = minPage; i <= maxPage; i++) input.push(i);
            return input;
        };
        //select page
        $scope.selectPage = function (page) {
            $scope.pagingOptions.pageNumber = page;
            $scope.updateGrid();
        };
        //next page
        $scope.nextPage = function () {
            if ($scope.pagingOptions.pageNumber < Math.ceil($scope.pagingOptions.totalRows / $scope.pagingOptions.pageSize)) {
                $scope.pagingOptions.pageNumber++;
                $scope.selectPage($scope.pagingOptions.pageNumber);
            }
        };
        //prev page
        $scope.prevPage = function () {
            if ($scope.pagingOptions.pageNumber > 1) {
                $scope.pagingOptions.pageNumber--;
                $scope.selectPage($scope.pagingOptions.pageNumber);
            }
        };
